...
# Subset positive outcomes (purchases)
Shopping.1 <- subset(Shopping, Revenue == TRUE)
cat("Positive observations (Revenue = TRUE) = ", nrow(Shopping.1), "\n")

# Subset negative outcomes (no purchases)
Shopping.0 <- subset(Shopping, Revenue == FALSE)
cat("Negative observations (Revenue = FALSE) = ", nrow(Shopping.0), "\n")

# Subsample the negatives to match the size of positives
Shopping.0.bal <- Shopping.0[sample(nrow(Shopping.0), nrow(Shopping.1)), ]
cat("Negative balanced observations = ", nrow(Shopping.0.bal), "\n")

# Row bind positives and balanced negatives
Shopping.bal <- rbind(Shopping.1, Shopping.0.bal)
