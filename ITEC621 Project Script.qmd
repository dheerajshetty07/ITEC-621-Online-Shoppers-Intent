...
(Creating the x predictor matrix and y outcome vector)

```{r}
library(glmnet)

x <- model.matrix(Revenue ~ ., data = Shopping.bal)[, -1]
y <- Shopping.bal$Revenue

lasso_model1 <- glmnet(x, y, alpha = 1, family = "binomial")
plot(lasso_model1, main = "LASSO Coefficients Path - Spec 1")
...
```{r}
set.seed(1)
lasso_cv2 <- cv.glmnet(x, y, alpha = 1, family = "binomial")

plot(lasso_cv2, main = "LASSO CV Error")

lasso_lambda2 <- lasso_cv2$lambda.min
lasso_coef2 <- coef(lasso_cv2, s = lasso_lambda2)
```

Lasso Coefficients Listed at the Optimal Lambda

```{r}
lasso_coef2 <- coef(lasso_cv2, s = lasso_lambda2)

lasso_coef_matrix <- round(as.matrix(lasso_coef2), 4)
...
library(knitr)
kable(lasso_coef_df, caption = "LASSO Coefficients at Optimal Lambda")
...
```{r}
lasso.best.lambda <- lasso_cv2$lambda.min
min.cv.lasso <- min(lasso_cv2$cvm)
